    ...
    for obj_handle in all_objects:
        try:
            ...
            if 'Cuboid' in obj_name:
                cuboids.append((obj_name, obj_handle))
        except:
            pass
    # Sort by name to ensure consistent ordering
    cuboids.sort(key=lambda x: x[0])
    for obj_name, obj_handle in cuboids:
        print(f"  Found cuboid: {obj_name}")
    return [h for _, h in cuboids]
...
def find_drop_targets():
    ...
    for obj_handle in all_objects:
        try:
            obj_name = sim.getObjectAlias(obj_handle)
            if 'Drop' in obj_name:
                targets.append((obj_name, obj_handle))
        except:
            pass
    # Sort by name to ensure consistent ordering
    targets.sort(key=lambda x: x[0])
    for obj_name, obj_handle in targets:
        print(f"  Found drop target: {obj_name}")
    return [h for _, h in targets]
