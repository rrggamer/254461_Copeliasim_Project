...
# ======== NEW: Trajectory Planning (Joint & Task) ========

# --- Time-scaling helper (smooth 0..1) ---
def s_curve_05_10(tau: float) -> float:
    tau = float(np.clip(tau, 0.0, 1.0))
    return 6*tau**5 - 15*tau**4 + 10*tau**3

# --- SO(3) exponential (pair with so3_log) ---
...
# --- Task-space straight line with orientation slerp ---
...
def sysCall_thread():
    print("===== UR5 Pick-and-Place (Task-space traj) =====")

    # --- Handles ---
    hdl_j   = get_ur5_joint_handles()
    hdl_base= sim.getObject("/UR5")
    hdl_box = first_existing(["/Cuboid"])   # box to pick
    hdl_drop= first_existing(["/Drop"])     # place target
    hdl_grip= first_existing(["/UR5/RG2/openCloseJoint"])  # Robotiq-like dummy
    hdl_att = first_existing(["/UR5/RG2/attachPoint", "/UR5/EndPoint"])  # parenting point

    sim.setObjectParent(hdl_box, -1, True)  # detach box to world
    ...
    # --- Preset a comfortable wrist orientation (optional) ---
    rotate_deg = {0: 90, 4: 90, 5: 90}
    for idx,deg in rotate_deg.items():
        sim.setJointTargetPosition(hdl_j[idx], deg*d2r)
    sim.switchThread()
    q_pre = np.array([sim.getJointPosition(hdl_j[i]) for i in range(6)], float)
    R_tilt = fk_ur5(q_pre)[:3,:3]

    # === PICK (Task-space straight line to the box pose with R_tilt) ===
    print("[PHASE] PICK")
    ...
    # Start from current EE pose
    T_start = fk_ur5(q_pre)
    q_at_pick = move_taskspace(q_pre, T_start, T_goal_pick, hdl_j,
                               Tf=2.0, dt=0.02, lam=0.08, dq_max=5*d2r, micro_steps=3)

    gripper_action(hdl_grip, 2)  # close
    time.sleep(0.3)
    sim.setObjectParent(hdl_box, hdl_att, True)

    # === PLACE (Task-space straight line to drop pose, pointing down) ===
    print("[PHASE] PLACE")
    ...
    gripper_action(hdl_grip, 1)  # open
    time.sleep(0.3)
    sim.setObjectParent(hdl_box, -1, True)

    # === RETURN HOME (Joint-space quintic, safe & smooth) ===
    print("[PHASE] RETURN HOME (joint-space)")
    q_now = q_at_place
    _ = move_jointspace(q_now, q_home, hdl_j, Tf=3.0, dt=0.02)
