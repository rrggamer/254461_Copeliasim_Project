...
# ---------- UR5 Kinematics Core ----------
def ur5_transforms(joint_rad: np.ndarray):
    """
    Build base-to-link transforms T0i for i=0..6 (0 is identity, 6 is wrist)
    and T0E (end effector). Returns (T_list[0..6], T0E).
    Uses same DH parameters and offsets as the original.
    """
    ...
    T = [np.eye(4)]
    T.append(T[0] @ T_01)  # T01
    T.append(T[1] @ T_12)  # T02
    T.append(T[2] @ T_23)  # T03
    T.append(T[3] @ T_34)  # T04
    T.append(T[4] @ T_45)  # T05
    T.append(T[5] @ T_56)  # T06

    T_6E = np.eye(4)       # EE coincides with frame 6
    T_0E = T[6] @ T_6E
...
def ur5_fk(joint_rad: np.ndarray):
    ...
    T_list, T_0E = ur5_transforms(joint_rad)
    pos = T_0E[:3, 3]
    R   = T_0E[:3, :3]
    eul = euler_from_rotmat_xyz(R)
    return pos, eul, T_0E

def ur5_jacobian(joint_rad: np.ndarray) -> np.ndarray:
    """
    Geometric Jacobian (6x6) at the end-effector, expressed in the base frame.
    All joints are revolute. Columns: [Jv; Jw] with
      Jv_i = cross(z_{i-1}, (p_e - p_{i-1}))
      Jw_i = z_{i-1}
    where i = 1..6 and indices use base=0.
    """
    T_list, T_0E = ur5_transforms(joint_rad)
...
def sysCall_thread():
    ...
    print("\n=== UR5 Forward Kinematics, Jacobian, and Orientation Display ===")
    ...
    while t < 10.0:
        ...
        setpoint = base * np.random.uniform(0.2, 1.0, size=6)

        # Command joints
        for h, q in zip(joint_handles, setpoint):
        ...
        print(f"  Orientation (deg):  Roll={fk_eul_deg[0]:7.2f}  Pitch={fk_eul_deg[1]:7.2f}  Yaw={fk_eul_deg[2]:7.2f}")
        print("-" * 65)
        print("COPPELIASIM Actual Values")
        print(f"  Position [m]:  X={sim_pos[0]:.4f}  Y={sim_pos[1]:.4f}  Z={sim_pos[2]:.4f}")
        print(f"  Orientation (deg):  Roll={sim_eul_deg[0]:7.2f}  Pitch={sim_eul_deg[1]:7.2f}  Yaw={sim_eul_deg[2]:7.2f}")
        print("-" * 65)
        print("Geometric Jacobian J (6x6, base frame @ EE):")
        np.set_printoptions(suppress=True)
        fmt = {'float_kind': lambda x: f"{x:9.5f}"}
        print(np.array2string(J, formatter=fmt, max_line_width=120))
        print("=" * 65)
